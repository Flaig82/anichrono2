...
import { GripVertical, X, Scissors, Info } from "lucide-react";
...
export default function EditableEntryRow({
  ...
  const hasAnilistId = entry.anilist_id != null;

  return (
    <div
      className={cn(
        "flex flex-col rounded-xl border bg-[#212121] transition-colors",
        borderColor,
    ...
    >
      <div className="flex items-center gap-2 p-3">
        ...
        {/* Info button (only when entry has an AniList ID) */}
        {hasAnilistId && onToggleExpand && (
          <button
      ...
      {isExpanded && hasAnilistId && (
        <EntryDetailPanel anilistId={entry.anilist_id!} />
      )}
    </div>
  );
}
